...
int lucpd_cfg_load_with_file(LucpdConfig_t* cfg, const char* config_file)
{
    ...
    {
        if (max_clients > 0 && max_clients <= 100)
        { // 限制合理范围
    ...
    {
        if (recv_timeout >= 100 && recv_timeout <= 10000)
        { // 100ms~10s
    ...
    {
        if (rate_limit >= 1000 && rate_limit <= 60000)
        { // 1~60秒
    ...
    {
        if (session_timeout >= 1000 && session_timeout <= 30000)
        { // 1~30秒
