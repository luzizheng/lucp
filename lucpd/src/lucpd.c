...
static void* session_thread(void* arg) {
    ...
    while (running && *sess->server_running) {
            ...
            if (ret == 0 && frame.msgType == LUCP_MTYP_UPLOAD_REQUEST) {
                ...
                if (sess->config->protocol.validate_version) { 
                    if (frame.version_major != LUCP_VER_MAJOR) {
                        lucp_frame_make(&reply, frame.seq_num, LUCP_MTYP_ACK_START, LUCP_STAT_FAILED, "Bad version", 11);
                ...
                lucp_frame_make(&reply, sess->seq_num, LUCP_MTYP_ACK_START, LUCP_STAT_SUCCESS, NULL, 0);
                lucp_net_send(&netctx, &reply);
                log_debug("[Session %d] State: INIT -> WAITING_UPLOAD_REQUEST, sent LUCP_MTYP_ACK_START.", sess->fd);
            } else if (ret < 0) {
                sess->state = LUCP_SESSION_ERROR;
            }
            break;
        ...
        case LUCP_SESSION_WAITING_UPLOAD_REQUEST: {
            ...
            lucp_frame_make(&reply, sess->seq_num, LUCP_MTYP_NOTIFY_DONE, prep_status, payload, strlen(payload));
            lucp_net_send(&netctx, &reply);
            log_debug("[Session %d] Sent LUCP_MTYP_NOTIFY_DONE(0x%x) (status=0x%x).", sess->fd,LUCP_MTYP_NOTIFY_DONE, prep_status);
        ...
        case LUCP_SESSION_WAITING_FTP_LOGIN_RESULT: {
            ...
            if (ret == 0 && frame.msgType == LUCP_MTYP_FTP_LOGIN_RESULT) {
                log_debug("[Session %d] Got LUCP_MTYP_FTP_LOGIN_RESULT(0x%x) (FTP login result, status=0x%x).", sess->fd, LUCP_MTYP_FTP_LOGIN_RESULT, frame.status);
        ...
        case LUCP_SESSION_WAITING_FTP_DOWNLOAD_RESULT: {
            ...
            if (ret == 0 && frame.msgType == LUCP_MTYP_FTP_DOWNLOAD_RESULT) {
                log_debug("[Session %d] Got LUCP_MTYP_FTP_DOWNLOAD_RESULT(0x%x) (Download result, status=0x%x).", sess->fd, LUCP_MTYP_FTP_DOWNLOAD_RESULT, frame.status);
                if (frame.status != LUCP_STAT_SUCCESS) {
                    sess->state = LUCP_SESSION_ERROR;
                } else {
...
int main(int argc, char **argv) {
    ...
    printf("[Server] Listening on %s:%d (max_clients=%d)\n",
           g_lucpdcfg.network.ip, g_lucpdcfg.network.port, g_lucpdcfg.network.max_clients);
